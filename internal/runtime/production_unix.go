...
func NewProductionHelper(ctx context.Context, version string, environ func(string) string) (*Helper, error) {
	...
	dialer, err := connector.NewFRPDialer(connector.FRPDialerConfig{ReadyTimeout: readyTimeout, RouteKinds: []string{"helper_https_wss", "preview_public_https_wss"}, PreferencePath: preferencePath})
	if err != nil {
		return nil, err
	}
