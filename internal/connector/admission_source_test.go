...
func validAdmissionResponse(input admissionRequest) string {
	encoded, _ := json.Marshal(admissionResponse{OperationID: input.OperationID, EnvironmentID: input.EnvironmentID, HelperID: input.HelperID, Generation: 3, EdgePool: input.EdgePool, EdgeNodeID: "edge_1", EdgeEndpoint: EdgeEndpoint{Host: "edge.test", Port: 7000}, Routes: []RouteHandoff{{RouteID: "route_1", Revision: 1, Kind: "helper_https_wss", PublicHost: "helper.test", ProxyName: "helper_1", LocalTarget: RouteTarget{Host: "127.0.0.1", Port: 8080}}}, ProtocolVersion: "1.0", Capabilities: []string{"terminal.v1"}, Credential: "test-only-connector-admission-credential"})
