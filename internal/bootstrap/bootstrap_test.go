...
func TestDashboardTokenPairingAndMaterialExchange(t *testing.T) {
	...
	server = httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/api/connected-machines/pairings":
			pairingCalls++
		...
		case "/api/connected-machines/pairings/installation":
			materialCalls++
			if materialCalls == 1 {
				writer.WriteHeader(http.StatusConflict)
				_ = json.NewEncoder(writer).Encode(map[string]any{"error": map[string]string{"code": "connected_machine_approval_pending", "message": "Machine approval is pending."}})
				return
			}
			manifest, publicKey := signedArtifact(t, server.URL, []byte("helper"))
			_ = json.NewEncoder(writer).Encode(map[string]any{"data": Material{Schema: "paperboat.byod-installation/v1", MachineID: "cm_1", MachineEnrollmentID: "cme_1", EnvironmentID: "env_1", ControlURL: server.URL, HelperID: "helper_1", EnrollmentID: "enroll_1", EnrollmentCredential: "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567ABCDEFGHIJKLMNOP", ExpiresAt: expires, Artifact: &manifest, ArtifactPublicKey: publicKey, HelperListenAddress: "127.0.0.1:38080"}})
		default:
...
func TestWaitForMaterialStopsOnTerminalServerErrors(t *testing.T) {
	tests := []struct {
		...
		{name: "denied", status: http.StatusForbidden, code: "connected_machine_pairing_denied", want: ErrPairingDenied},
		{name: "expired", status: http.StatusGone, code: "connected_machine_pairing_expired", want: ErrPairingExpired},
		{name: "unavailable", status: http.StatusGone, code: "connected_machine_installation_unavailable", want: ErrInstallationUnavailable},
